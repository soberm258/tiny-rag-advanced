...
def main():
    # 小数点不应断句：例如“面积2.4万平方公里”应保持为同一句
    ss_small = SentenceSplitter(use_model=False, sentence_size=2048)
    demo = "滁州市琅琊区、南谯区、来安县和天长市，宣州区，面积2.4万平方公里。"
    demo_res = ss_small.split_text(demo)
    assert any("2.4万平方公里" in s for s in demo_res), demo_res
